...
class moderator extends db_connect
{
    ...
    public function postPhoto($accountId, $imageName = "")
    {
        ...
        if ($stmt->execute()) {
            ...
            $notify = new notifications($this->db);
            $notify->remove(NOTIFY_TYPE_PROFILE_PHOTO_APPROVE, $accountId);
            $notify->remove(NOTIFY_TYPE_PROFILE_PHOTO_REJECT, $accountId);
            unset($notify);
        }
    ...
    public function approvePhoto($accountId, $notification = false)
    {
        ...
        if ($stmt->execute()) {
            ...
            if ($notification) {
                ...
                $notify = new notifications($this->db);
                $notify->create($accountId, NOTIFY_TYPE_PROFILE_PHOTO_APPROVE, $accountId);
                unset($notify);
            }
    ...
    public function rejectPhoto($accountId)
    {
        ...
        if ($stmt->execute()) {
            ...
            $notify = new notifications($this->db);
            $notify->create($accountId, NOTIFY_TYPE_PROFILE_PHOTO_REJECT, $accountId);
            unset($notify);
        }
    ...
    public function postCover($accountId, $imageName = "")
    {
        ...
        if ($stmt->execute()) {
            ...
            $notify = new notifications($this->db);
            $notify->remove(NOTIFY_TYPE_PROFILE_COVER_APPROVE, $accountId);
            $notify->remove(NOTIFY_TYPE_PROFILE_COVER_REJECT, $accountId);
            unset($notify);
        }
    ...
    public function approveCover($accountId, $notification = false)
    {
        ...
        if ($stmt->execute()) {
            ...
            if ($notification) {
                ...
                $notify = new notifications($this->db);
                $notify->create($accountId, NOTIFY_TYPE_PROFILE_COVER_APPROVE, $accountId);
                unset($notify);
            }
    ...
    public function rejectCover($accountId)
    {
        ...
        if ($stmt->execute()) {
            ...
            $notify = new notifications($this->db);
            $notify->create($accountId, NOTIFY_TYPE_PROFILE_COVER_REJECT, $accountId);
            unset($notify);
