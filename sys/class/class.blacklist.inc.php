...
class blacklist extends db_connect
{
    ...
    {
        $stmt = $this->db->prepare("SELECT id FROM profile_blacklist WHERE blockedByUserId = (:blockedByUserId) AND blockedUserId = (:blockedUserId) AND removeAt = 0 LIMIT 1");
        $stmt->bindParam(":blockedByUserId", $this->requestFrom, PDO::PARAM_INT);
        $stmt->bindParam(":blockedUserId", $userId, PDO::PARAM_INT);
    ...
    private function itemInfo($itemId)
    {
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $blockedUserId = $profile->getVeryShort();
                unset($profile);
                ...
                $result = array("error" => false,
                                ...
                                "blockedUserVerified" => $blockedUserId['verified'],
                                ...
                                "blockedUserPhotoUrl" => $blockedUserId['lowThumbnailUrl'],
