...
class reports extends db_connect
{
    ...
    public function add($itemType, $itemId, $abuseId, $description = "")
    {
        ...
        $itemInfo = array('error' => true);

        $toUseId = 0;

        if ($itemType == REPORT_TYPE_ITEM) {

            $items = new items($this->db);
            $itemInfo = $items->info($itemId);

            if ($itemInfo['error'] || $itemInfo['removeAt'] != 0 || $itemInfo['rejectedAt'] != 0) {

                return $result;

            } else {

                $toUseId = $itemInfo['fromUserId'];
            }
    ...
    public function quickInfo($row)
    {
        ...
        $result = array(
            ...
            "toUserUsername" => $profileInfo['username'],
            "toUserFullname" => $profileInfo['fullname'],
            "toUserPhotoUrl" => $profileInfo['lowThumbnailUrl'],
            "toUserOnline" => $profileInfo['online'],
            "toUserVerified" => $profileInfo['verified'],
            "toUserLocation" => $profileInfo['location'],
            "itemId" => $row['itemId'],
            "abuseId" => $row['abuseId'],
        ...
        if ($row['toUserId'] != 0) {
            ...
            $result['toUserUsername'] = $profileInfo['username'];
            $result['toUserFullname'] = $profileInfo['fullname'];
            $result['toUserPhotoUrl'] = $profileInfo['lowThumbnailUrl'];
            $result['toUserOnline'] = $profileInfo['online'];
            $result['toUserVerified'] = $profileInfo['verified'];
            $result['toUserLocation'] = $profileInfo['location'];
        }
